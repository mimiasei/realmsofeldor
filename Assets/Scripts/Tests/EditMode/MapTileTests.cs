...
namespace RealmsOfEldor.Tests
{
    public class MapTileTests
    {
        ...
        {
            var tile = new MapTile(TerrainType.GrassTemperate, 0, 100);
        ...
        {
            var tile = new MapTile(TerrainType.GrassTemperate);
        ...
        {
            var tile = new MapTile(TerrainType.RockTemperate);
            Assert.IsTrue(tile.IsPassable()); // Rock is passable in new system
        }

        [Test]
        public void IsPassable_ReturnsTrueForSnow()
        {
            var tile = new MapTile(TerrainType.SnowTemperate);
            Assert.IsTrue(tile.IsPassable()); // Snow is passable (Subterranean removed)
        }
        ...
        {
            var tile = new MapTile(TerrainType.GrassTemperate);
            tile.SetTerrain(TerrainType.SandTemperate, 1, 150);

            Assert.AreEqual(TerrainType.SandTemperate, tile.Terrain);
        ...
        {
            var tile = new MapTile(TerrainType.GrassTemperate, 2, 100);
            tile.SetTerrain(TerrainType.DirtTemperate, -1, -1);

            Assert.AreEqual(TerrainType.DirtTemperate, tile.Terrain);
